...
wlprompt is a tool to start programs under wayland with tab completions. Think of it as a shell
prompt without having to start a terminal (but also without most of the shell features). Inspired by
[AwesomeWM](https://awesomewm.org/)'s `awful.widget.prompt`.
...
* Provide an example of code using extremely poorly documented smithay-client-toolkit for myself and
  hopefully others;
* Play / gain some experience with Rust;
...
3. Run `cargo build` or `cargo install`;
